...
export const generarPDF = (datos) => {
  ...
  // --- Totales ---
  doc.setFontSize(12);
  doc.text(`Subtotal: $${datos.subtotal.toFixed(2)}`, 140, y + 10, { align: 'right' });
  doc.text(`IVA (${datos.ivaPorcentaje}%): $${datos.iva.toFixed(2)}`, 140, y + 17, { align: 'right' });
  doc.setFontSize(14);
  doc.setTextColor(0, 0, 0);
  doc.text(`TOTAL: $${datos.total.toFixed(2)}`, 140, y + 27, { align: 'right' });
